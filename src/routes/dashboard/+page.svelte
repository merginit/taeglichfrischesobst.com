...
{#if loginState && gigLocationSuggestionsHandler && gigLetterSubscribersHandler}
	<div
		class="absolute overflow-hidden cursor-pointer right-2 top-2"
		on:click={() => initializeTables()}
	>
					...
					<tbody class="text-black">
						{#each $gigLocationSuggestions as suggestion}
							<tr>
								...
								<td class="p-4"
									><a href="mailto:{suggestion?.email}">{suggestion?.email ?? ''}</a></td
								>
							</tr>
						{:else}
				...
				<table class="table">
					...
					<tbody class="text-black">
						{#each $gigLetterSubscribers as subscriber}
							<tr>
								<td class="p-4"
									><a href="mailto:{subscriber?.key ?? ''}">{subscriber?.key ?? ''}</a></td
								>
			...
			<div class="flex flex-col gap-2">
				...
				>
					<input
						type="password"
						name="password"
						class="bg-white input input-bordered input-accent"
						title="password"
						placeholder="*****"
					/>
					<button type="submit" class="btn-accent btn">login</button>
				</form>
			</div>
		</div>
	{/if}
