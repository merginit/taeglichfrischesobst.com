...
export const POST = (async ({ request }) => {
	...
	if (emailRegex.test(email)) {
		...
		if (value) {
			...
			return new Response(
				JSON.stringify({ success, response: 'Du wurdest erfolgreich entfernt!', objres: response })
			);
		} else {
			return new Response(
				JSON.stringify({ success: false, response: 'Du bist nicht auf der Liste!', objres: null })
			);
		}
	} else {
		return new Response(
