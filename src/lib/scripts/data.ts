...
export const fetchImages = async function () {
	...
	const fetchedImagesAsImagesType = fetchedImages.map((image: StoryblokImageAsset) => {
		const dimensions = (image.webp?.filename || image.png?.filename)?.split('/')[5]?.split('x'); // get image dimensions from src
		const splitName = (image.webp?.filename || image.png?.filename)?.split('-');
		const slicedName = splitName.slice(-2);
		const couldBeAuthor = `${slicedName[0]?.replace(' ', '')} ${slicedName[1]
			?.replace('.webp', '')
			?.replace('.png', '')}`; // get author from src
		...
		return {
			...
			webp: {
				...
				alt:
					image.webp?.alt ??
					image.webp?.title ??
					image.webp?.filename?.split('©')[1]?.split('.')[0],
			...
			png: {
				...
				alt:
					image.png?.alt ?? image.png?.title ?? image.png?.filename?.split('©')[1]?.split('.')[0],
